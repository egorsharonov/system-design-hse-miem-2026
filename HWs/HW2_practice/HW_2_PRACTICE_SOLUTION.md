...
> ![img_4.png](img_4.png)
> Скриншот 4. Проверка количества записей в `owners` и `events` после применения SQL-скрипта и первых операций генератора.
...
> ![img_9.png](img_9.png)
> Скриншот 9. Генератор трафика во время failover: зафиксирована временная потеря write-соединения.
...
После остановки лидера Patroni выбирает нового primary из реплик, а HAProxy переключает write endpoint на новую primary-ноду. Для клиента это проявляется как короткий разрыв соединения; дальнейшая доступность подтверждается новым лидером в `patronictl list` и переключением primary backend в HAProxy.
